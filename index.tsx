// Fix: Convert the file to a module to allow global scope augmentation. This resolves all errors related to properties not existing on `window`.
export {};
...
*/
// Fix: Allow attaching custom properties to the window object to resolve errors with functions called from HTML (onclick).
declare global {
    interface Window { [key: string]: any; }
}

// Fix: Added type definitions for data models to resolve 'property does not exist on type unknown' and improve type safety.
interface User {
  username: string;
  password?: string;
  type: 'teacher' | 'admin';
...
let currentParent: Student | null = null;
let currentClassId: string | null = null;
let classes: Class[] = [];
let students: { [classId: string]: { [studentId: string]: Student } } = {};
let rewards: { [classId: string]: Reward[] } = {};
let powerups: { [classId: string]: PowerUp[] } = {};
// Fix: Renamed 'history' to 'pointsHistory' to avoid conflict with window.history.
let pointsHistory: { [classId: string]: HistoryItem[] } = {};
let reasons: { [classId: string]: string[] } = {};
let embeds: { [school: string]: Embed[] } = {};
...
let currentReasonIndex: number | '' = '';
let currentView = 'grid';
let currentUsernameToEdit = '';
let emojiPickerTarget: HTMLInputElement | null = null;
let demoMode = false;
...
const sizeLevelMap: { [key: number]: string } = {
    "-2": "Smallest",
    "-1": "Small",
    "0": "Normal",
    "1": "Large",
...
// Fix: Attaching functions to window object and defining them early to be accessible globally.
...
window.showLogin = () => {
  (document.getElementById('signupScreen') as HTMLElement).style.display = 'none';
  (document.getElementById('studentLoginScreen') as HTMLElement).style.display = 'none';
  (document.getElementById('parentLoginScreen') as HTMLElement).style.display = 'none';
...
window.showSignup = () => {
  ...
  (document.getElementById('studentLoginScreen') as HTMLElement).style.display = 'none';
  (document.getElementById('parentLoginScreen') as HTMLElement).style.display = 'none';
  (document.getElementById('signupScreen') as HTMLElement).style.display = 'flex';
}
...
window.showStudentLogin = () => {
    ...
    (document.getElementById('signupScreen') as HTMLElement).style.display = 'none';
    (document.getElementById('parentLoginScreen') as HTMLElement).style.display = 'none';
    (document.getElementById('studentLoginScreen') as HTMLElement).style.display = 'flex';
}
...
window.showParentLogin = () => {
    ...
    (document.getElementById('signupScreen') as HTMLElement).style.display = 'none';
    (document.getElementById('studentLoginScreen') as HTMLElement).style.display = 'none';
    (document.getElementById('parentLoginScreen') as HTMLElement).style.display = 'flex';
}

window.selectAccountType = (type: string) => {
...
function signup() {
  ...
  const type = (document.querySelector('.account-type-btn.active') as HTMLElement).dataset.type as 'teacher' | 'admin';
  // Fix: Cast elements to HTMLInputElement to access value/files properties.
  ...
  const school = (document.getElementById('signupSchool') as HTMLInputElement).value.trim();
  const email = (document.getElementById('signupEmail') as HTMLInputElement).value.trim().toLowerCase();
  const photoFile = (document.getElementById('signupPhoto') as HTMLInputElement).files?.[0];
  ...
  // Fix: Explicitly type 'u' to 'User' to access 'email' property.
  if (Object.values(users).some((u: User) => u.email === email)) {
    error.textContent = 'Email address is already in use';
    error.style.display = 'block';
    return;
  }

  const newUser: User = {username, password, type, school, email, createdAt: new Date().toISOString(), settings: { allowStudentPins: true, allowParentPins: true }};
  ...
  if (photoFile) {
    ...
    reader.onload = (e) => {
        newUser.photo = e.target?.result as string;
        finalizeSignup();
    };
...
function loginWithPIN() {
    const pin = (document.getElementById('studentPin') as HTMLInputElement).value.trim().toUpperCase();
    const error = document.getElementById('studentLoginError') as HTMLElement;
    error.style.display = 'none';
...
function loginWithParentPIN() {
    const pin = (document.getElementById('parentPin') as HTMLInputElement).value.trim().toUpperCase();
    const error = document.getElementById('parentLoginError') as HTMLElement;
    error.style.display = 'none';
...
function initializeApp() {
  ...
  (document.getElementById('sidebarUsername') as HTMLElement).textContent = currentUser.username;
  (document.getElementById('sidebarSchool') as HTMLElement).textContent = currentUser.school;

  const avatarDiv = document.getElementById('sidebarUserAvatar') as HTMLElement;
...
function initializeAppStudent() {
    ...
    (document.getElementById('studentPortalName') as HTMLElement).textContent = currentStudent.name;
    (document.getElementById('studentPortalPoints') as HTMLElement).textContent = `${currentStudent.points} pts`;
    currentClassId = currentStudent.classId!;
    // Fix: create a mock event object for the function call
    window.switchPortalView({ currentTarget: document.querySelector('#studentPortalTabs .modal-tab[data-view="leaderboard"]') } as unknown as Event);
}
...
function initializeAppParent() {
    ...
    (document.getElementById('parentPortalName') as HTMLElement).textContent = `Parent of ${currentParent.name}`;
    (document.getElementById('parentPortalPoints') as HTMLElement).textContent = `${currentParent.points} pts`;
    currentClassId = currentParent.classId!;
    const container = document.getElementById('parentPortalContent') as HTMLElement;
    container.innerHTML = '';
    ...
    // Fix: Call renderHistoryView with both arguments
    renderHistoryView(studentHistory, container);
}

window.switchPortalView = (event: Event) => {
    const targetView = ((event.currentTarget) as HTMLElement).dataset.view;
    document.querySelectorAll('#studentPortalTabs .modal-tab').forEach(t => t.classList.remove('active'));
    (event.currentTarget as HTMLElement).classList.add('active');

    const contentArea = document.getElementById('studentPortalContent') as HTMLElement;
    contentArea.innerHTML = '';

    if (targetView === 'leaderboard') {
        // Fix: Call renderLeaderboard with a container
...
function router() {
    ...
    if (currentUser.type === 'admin') {
        ...
        (document.getElementById('classSelector') as HTMLElement).style.display = 'none';
        (document.querySelector('.topbar-right .btn-primary') as HTMLElement).style.display = 'none';
        (document.querySelector('.topbar-left .btn-success') as HTMLElement).style.display = 'none';
    } else {
        defaultView = 'students';
        (document.getElementById('classSelector') as HTMLElement).style.display = 'block';
        (document.querySelector('.topbar-right .btn-primary') as HTMLElement).style.display = 'block';
        (document.querySelector('.topbar-left .btn-success') as HTMLElement).style.display = 'block';
        updateClassSelector();
    }
...
function updateNavItems() {
  if (!currentUser) return;
  const nav = document.getElementById('sidebarNav') as HTMLElement;
  nav.innerHTML = '';
...
function switchView(view: string) {
  ...
  document.querySelectorAll('.page-view').forEach(d => (d as HTMLElement).style.display = 'none');
  const title = view.split('?')[0]; // handle query params if any
  (document.getElementById('pageTitle') as HTMLElement).textContent = title.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase());
  (document.getElementById('searchInput') as HTMLElement).style.display = 'none';
  ...
  if (currentUser.type === 'admin') {
    ...
    if (currentClassId) {
        ...
        // Fix: Call renderHistoryView with both arguments
        else if(title === 'history') renderHistoryView(pointsHistory[currentClassId] || [], document.getElementById('historyView') as HTMLElement);
        else if(title === 'settings') renderSettingsView();
        else if(title === 'powerups') renderPowerUpsView();
    } else if (title === 'students') {
        (document.getElementById('studentsGrid') as HTMLElement).innerHTML = `
        ...
        (document.getElementById('studentsCompact') as HTMLElement).innerHTML = '';
        (document.getElementById('studentListTable')!.querySelector('tbody') as HTMLElement).innerHTML = '';
    }
    
    if (title === 'extras') renderExtrasView();

    if (title === 'students') {
      (document.getElementById('searchInput') as HTMLElement).style.display = 'block';
      (document.getElementById('pageTitle') as HTMLElement).textContent = 'Students';
    }
  }
...
function triggerConfetti() {
    ...
    if (!canvas) return;
    // Fix: Cast to CanvasRenderingContext2D to access canvas methods.
...
function updateClassSelector() {
  if (!currentUser) return;
  const selector = document.getElementById('classSelector') as HTMLSelectElement;
  selector.innerHTML = '';
...
window.addNewClass = () => {
  const className = (document.getElementById('newClassName') as HTMLInputElement).value.trim();
  if (!className) {
...
function getPowerUpInfo(studentId: string) {
...
function renderStudentsView() {
  ...
  const grid = document.getElementById('studentsGrid') as HTMLElement;
  const listTableBody = document.getElementById('studentListTable')!.querySelector('tbody') as HTMLElement;
  const compact = document.getElementById('studentsCompact') as HTMLElement;
  ...
  (document.getElementById('managePinsBtn') as HTMLElement).style.display = (teacherSettings?.allowStudentPins || teacherSettings?.allowParentPins) ? 'inline-flex' : 'none';

  window.onSearchChange();
}

window.switchStudentView = (event: Event, view: string) => {
  document.querySelectorAll('.view-option').forEach(opt => opt.classList.remove('active'));
  (event.currentTarget as HTMLElement).classList.add('active');
  const listControls = document.querySelector('.list-view-controls') as HTMLElement;
  const gridControls = document.querySelector('.grid-columns-selector') as HTMLElement;

  (document.getElementById('studentsGrid') as HTMLElement).style.display = view === 'grid' ? 'grid' : 'none';
  (document.getElementById('studentsList') as HTMLElement).style.display = view === 'list' ? 'block' : 'none';
  (document.getElementById('studentsCompact') as HTMLElement).style.display = view === 'compact' ? 'grid' : 'none';
...
function updateListViewSize() {
    const table = document.getElementById('studentListTable') as HTMLElement;
    const display = document.getElementById('listSizeDisplay') as HTMLElement;
    const increaseBtn = document.getElementById('increaseListSizeBtn') as HTMLButtonElement;
    const decreaseBtn = document.getElementById('decreaseListSizeBtn') as HTMLButtonElement;

    // Fix: Convert number to string for dataset property.
...
window.setGridColumns = (val: string) => {
  const grid = document.getElementById('studentsGrid') as HTMLElement;
  if (val === 'auto') {
...
window.openAddStudentModal = () => {
  ...
  (document.getElementById('initialPoints') as HTMLInputElement).value = '0';
  (document.getElementById('studentPhoto') as HTMLInputElement).value = '';
  (document.getElementById('photoPreview') as HTMLElement).style.display = 'none';
  ((document.getElementById('photoPreview') as HTMLElement).querySelector('img') as HTMLImageElement).src = '';
  window.openModal('addStudentModal');
}

window.previewPhoto = (event: Event, previewId: string) => {
  const file = (event.target as HTMLInputElement).files?.[0];
  if (file) {
    const reader = new FileReader();
    reader.onload = function(e) {
      // Fix: Ensure result is a string before assigning to src.
      if (e.target?.result) {
        ((document.getElementById(previewId) as HTMLElement).querySelector('img') as HTMLImageElement).src = e.target.result as string;
      }
      (document.getElementById(previewId) as HTMLElement).style.display = 'block';
    };
...
window.addStudent = () => {
  ...
  const initialPoints = parseInt((document.getElementById('initialPoints') as HTMLInputElement).value) || 0;
  const file = (document.getElementById('studentPhoto') as HTMLInputElement).files?.[0];
  ...
  if (file) {
    const reader = new FileReader();
    reader.onload = function(e) {
      student.photo = e.target?.result as string;
...
function finalizeAddStudent(student: Student) {
  if (!currentClassId) return;
  if (!students[currentClassId]) students[currentClassId] = {};
...
window.openAddMultipleModal = () => {
  (document.getElementById('multipleNames') as HTMLTextAreaElement).value = '';
  window.openModal('addMultipleModal');
}

window.addMultipleStudents = () => {
  if (!currentClassId) return;
  const names = (document.getElementById('multipleNames') as HTMLTextAreaElement).value.trim().split('\n').map(n => n.trim()).filter(n => n);
  names.forEach(name => {
...
window.enterBulkMode = () => {
  ...
  (document.getElementById('selectedCount') as HTMLElement).textContent = '0 selected';
...
window.toggleStudentSelection = (studentId: string) => {
  if (!bulkMode) return;
  ...
  (document.getElementById('selectedCount') as HTMLElement).textContent = `${selectedStudents.size} selected`;
  renderStudentsView();
}
...
window.openBulkPointsModal = (action: 'add' | 'subtract') => {
  bulkAction = action;
  (document.getElementById('bulkModalTitle') as HTMLElement).textContent = action === 'add' ? 'Bulk Add Points' : 'Bulk Subtract Points';
  (document.getElementById('bulkSummary') as HTMLElement).textContent = `Apply to ${selectedStudents.size} selected students`;
  (document.getElementById('bulkReason') as HTMLInputElement).value = '';
  const amount = parseInt((document.getElementById('bulkPointAmount') as HTMLInputElement).value);
  (document.getElementById('bulkModalTitle') as HTMLElement).textContent += ` (${action === 'add' ? '+' : '-'}${amount} pts)`;
...
window.confirmBulkPoints = () => {
  const amount = parseInt((document.getElementById('bulkPointAmount') as HTMLInputElement).value);
  const reason = (document.getElementById('bulkReason') as HTMLInputElement).value.trim();
  if (!amount) {
...
window.openStudentPointsModal = (studentId: string) => {
  currentStudentId = studentId;
  if (!currentClassId) return;
  const student = students[currentClassId][studentId];
  (document.getElementById('studentModalTitle') as HTMLElement).textContent = `Change points for ${sanitizeHTML(student.name)}`;
  (document.getElementById('pointsChange') as HTMLInputElement).value = '1';
  (document.getElementById('pointsReason') as HTMLInputElement).value = '';
...
window.updateStudentPoints = () => {
  ...
  let change = parseInt((document.getElementById('pointsChange') as HTMLInputElement).value);
  const reason = (document.getElementById('pointsReason') as HTMLInputElement).value.trim();
  if (isNaN(change)) {
...
window.openEditStudentModal = (studentId: string) => {
  currentStudentId = studentId;
  if (!currentClassId) return;
  const student = students[currentClassId][studentId];
  (document.getElementById('editStudentName') as HTMLInputElement).value = student.name;
  (document.getElementById('editStudentPhoto') as HTMLInputElement).value = '';
  const preview = document.getElementById('editPhotoPreview') as HTMLElement;
...
window.updateStudent = () => {
  ...
  const file = (document.getElementById('editStudentPhoto') as HTMLInputElement).files?.[0];
  if (!name) {
...
function renderLeaderboardView() {
    const view = document.getElementById('leaderboardView') as HTMLElement;
    view.innerHTML = `
...
function renderLeaderboard(container?: HTMLElement) {
...
// Rewards Logic
function renderRewardsView(targetContainer?: HTMLElement) {
  if (!currentClassId) return;
  if (!rewards[currentClassId]) rewards[currentClassId] = [];
  const classRewards = rewards[currentClassId];
  const grid = targetContainer || document.getElementById('rewardsGrid') as HTMLElement;
  grid.innerHTML = '';
...
window.openCreateRewardModal = () => {
  (document.getElementById('rewardTitle') as HTMLInputElement).value = '';
  (document.getElementById('rewardDescription') as HTMLInputElement).value = '';
  (document.getElementById('rewardPoints') as HTMLInputElement).value = '';
  (document.getElementById('rewardIcon') as HTMLInputElement).value = '';
  (document.querySelector('input[name="rewardType"][value="deduct"]') as HTMLInputElement).checked = true;
  window.openModal('createRewardModal');
}

window.createReward = () => {
  const title = (document.getElementById('rewardTitle') as HTMLInputElement).value.trim();
  const description = (document.getElementById('rewardDescription') as HTMLTextAreaElement).value.trim();
  const points = parseInt((document.getElementById('rewardPoints') as HTMLInputElement).value);
  const type = (document.querySelector('input[name="rewardType"]:checked') as HTMLInputElement).value as 'deduct' | 'require';
  const icon = (document.getElementById('rewardIcon') as HTMLInputElement).value.trim();

  if (!title || isNaN(points)) {
    showNotification('Title and points are required', 'error');
    return;
  }
  const reward: Reward = { id: generateId(), title, description, points, type, icon };
  if (!currentClassId) return;
  if (!rewards[currentClassId]) rewards[currentClassId] = [];
...
window.openEditRewardModal = (rewardId: string) => {
  currentRewardId = rewardId;
  if (!currentClassId) return;
  const reward = rewards[currentClassId].find(r => r.id === rewardId)!;
  (document.getElementById('editRewardTitle') as HTMLInputElement).value = reward.title;
  (document.getElementById('editRewardDescription') as HTMLTextAreaElement).value = reward.description;
  (document.getElementById('editRewardPoints') as HTMLInputElement).value = String(reward.points);
  (document.getElementById('editRewardIcon') as HTMLInputElement).value = reward.icon;
  (document.querySelector(`input[name="editRewardType"][value="${reward.type}"]`) as HTMLInputElement).checked = true;
...
window.updateReward = () => {
  ...
  const title = (document.getElementById('editRewardTitle') as HTMLInputElement).value.trim();
  const description = (document.getElementById('editRewardDescription') as HTMLTextAreaElement).value.trim();
  const points = parseInt((document.getElementById('editRewardPoints') as HTMLInputElement).value);
  const type = (document.querySelector('input[name="editRewardType"]:checked') as HTMLInputElement).value as 'deduct' | 'require';
  const icon = (document.getElementById('editRewardIcon') as HTMLInputElement).value.trim();
...
window.openClaimRewardModal = (rewardId: string) => {
  currentRewardId = rewardId;
  if (!currentClassId) return;
  const reward = rewards[currentClassId].find(r => r.id === rewardId)!;
  const content = document.getElementById('claimRewardContent') as HTMLElement;
...
window.toggleStudentSelectionInModal = (element: HTMLElement) => {
    element.classList.toggle('selected');
}

window.toggleSelectAllStudents = (select: boolean) => {
...
window.confirmClaimReward = () => {
  if (!currentClassId) return;
  const selectedStudentIds = Array.from(document.querySelectorAll('#claimRewardContent .selectable-student-card.selected')).map(el => (el as HTMLElement).dataset.studentId as string);
...
// Power Ups Logic
function renderPowerUpsView(targetContainer?: HTMLElement) {
  if (!currentClassId) return;
  if (!powerups[currentClassId]) powerups[currentClassId] = [];
  const classPowerups = powerups[currentClassId];
  const grid = targetContainer || document.getElementById('powerupsGrid') as HTMLElement;
  grid.innerHTML = '';
...
window.openCreatePowerUpModal = () => {
  (document.getElementById('powerUpTitle') as HTMLInputElement).value = '';
  (document.getElementById('powerUpDescription') as HTMLTextAreaElement).value = '';
  (document.getElementById('powerUpMultiplier') as HTMLInputElement).value = '1.5';
  (document.getElementById('powerUpDuration') as HTMLInputElement).value = '7';
  window.openModal('createPowerUpModal');
}

window.createPowerUp = () => {
  const title = (document.getElementById('powerUpTitle') as HTMLInputElement).value.trim();
  const description = (document.getElementById('powerUpDescription') as HTMLTextAreaElement).value.trim();
  const multiplier = parseFloat((document.getElementById('powerUpMultiplier') as HTMLInputElement).value);
  const duration = parseInt((document.getElementById('powerUpDuration') as HTMLInputElement).value);

  if (!title || isNaN(multiplier) || isNaN(duration)) {
    showNotification('All fields are required', 'error');
    return;
  }
  const powerup: PowerUp = { id: generateId(), title, description, multiplier, duration };
  if (!currentClassId) return;
  if (!powerups[currentClassId]) powerups[currentClassId] = [];
...
window.openEditPowerUpModal = (powerupId: string) => {
  currentPowerUpId = powerupId;
  if (!currentClassId) return;
  const powerup = powerups[currentClassId].find(p => p.id === powerupId)!;
  (document.getElementById('editPowerUpTitle') as HTMLInputElement).value = powerup.title;
  (document.getElementById('editPowerUpDescription') as HTMLTextAreaElement).value = powerup.description;
  (document.getElementById('editPowerUpMultiplier') as HTMLInputElement).value = String(powerup.multiplier);
  (document.getElementById('editPowerUpDuration') as HTMLInputElement).value = String(powerup.duration);
...
window.updatePowerUp = () => {
    ...
    const title = (document.getElementById('editPowerUpTitle') as HTMLInputElement).value.trim();
    const description = (document.getElementById('editPowerUpDescription') as HTMLTextAreaElement).value.trim();
    const multiplier = parseFloat((document.getElementById('editPowerUpMultiplier') as HTMLInputElement).value);
    const duration = parseInt((document.getElementById('editPowerUpDuration') as HTMLInputElement).value);
...
window.assignPowerUp = (powerupId: string) => {
  currentPowerUpId = powerupId;
  if (!currentClassId) return;
  const powerup = powerups[currentClassId].find(p => p.id === powerupId)!;
  const content = document.getElementById('claimRewardContent') as HTMLElement;
  ...
  const modal = document.getElementById('claimRewardModal') as HTMLElement;
  (modal.querySelector('.modal-title') as HTMLElement).textContent = 'Assign Power Up';
  (modal.querySelector('.modal-footer .btn-primary') as HTMLElement).textContent = 'Assign';
  (modal.querySelector('.modal-footer .btn-primary') as HTMLElement).onclick = window.confirmAssignPowerUp;
  window.openModal('claimRewardModal');
}
...
// History Logic
window.switchHistoryView = (event: Event, view: string) => {
    (event.currentTarget as HTMLElement).parentElement!.querySelector('.active')?.classList.remove('active');
    (event.currentTarget as HTMLElement).classList.add('active');
    const parent = (event.currentTarget as HTMLElement).closest('.page-view, .portal-main, .modal-tab-content') as HTMLElement;
    if (view === 'list') {
        (parent.querySelector('#historyListContainer') as HTMLElement).style.display = 'block';
        (parent.querySelector('#historyCalendarContainer') as HTMLElement).style.display = 'none';
    } else {
        (parent.querySelector('#historyListContainer') as HTMLElement).style.display = 'none';
        (parent.querySelector('#historyCalendarContainer') as HTMLElement).style.display = 'block';
    }
}

function renderHistoryView(historyData: HistoryItem[], container: HTMLElement) {
  container.innerHTML = `
  ...
  // Render List
  const listContainer = container.querySelector('#historyListContainer') as HTMLElement;
  listContainer.innerHTML = '';
  ...
  // Render Calendar
  (container.querySelector('#historyCalendarContainer') as HTMLElement).appendChild(renderCalendar(historyData || []));
  
  return container;
}

function renderCalendar(historyData: HistoryItem[]) {
    ...
    const historyByDay: { [key: string]: HistoryItem[] } = {};
    ...
    (container.querySelector('#prev-month') as HTMLElement).onclick = () => { historyCalendarDate.setMonth(month - 1); container.replaceWith(renderCalendar(historyData)); };
    (container.querySelector('#next-month') as HTMLElement).onclick = () => { historyCalendarDate.setMonth(month + 1); container.replaceWith(renderCalendar(historyData)); };

    return container;
}

function showDayHistoryPopup(event: MouseEvent, dayHistory: HistoryItem[]) {
...
window.clearHistory = () => {
  if (confirm('Are you sure you want to clear all point history for this class? This cannot be undone.')) {
    ...
    saveData();
    renderHistoryView(pointsHistory[currentClassId], document.getElementById('historyView') as HTMLElement);
...
function renderSettingsView() {
    if (!currentUser) return;
    const view = document.getElementById('settingsView') as HTMLElement;
    view.innerHTML = `
    ...
    if (currentUser.type === 'admin') {
        ...
        `;
        const avatarDiv = document.getElementById('settingsAvatar') as HTMLElement;
        if (currentUser.photo) {
            avatarDiv.innerHTML = `<img src="${currentUser.photo}" alt="Profile Picture"/>`;
        } else {
            avatarDiv.innerHTML = currentUser.username.charAt(0).toUpperCase();
        ...
        if (currentClassId) {
            ...
            if (currentClass) {
                (document.getElementById('currentClassName') as HTMLElement).textContent = currentClass.name;
                const classStudents = Object.values(students[currentClassId] || {});
                (document.getElementById('totalStudents') as HTMLElement).textContent = String(classStudents.length);
                (document.getElementById('totalPoints') as HTMLElement).textContent = String(classStudents.reduce((sum, s) => sum + s.points, 0));
            }
        }
        (document.getElementById('offlineModeToggle') as HTMLInputElement).checked = currentUser.settings?.offlineMode || false;
...
window.changePassword = () => {
    ...
    const confirmPass = (document.getElementById('confirmNewPassword') as HTMLInputElement).value;
...
function updateReasonsList() {
  const datalist = document.getElementById('reasonsList') as HTMLDataListElement;
  datalist.innerHTML = '';
...
function renderSavedReasons(containerId: string, inputId: string) {
    ...
    (reasons[currentClassId!] || []).forEach(reason => {
        ...
        chip.onclick = () => { (document.getElementById(inputId) as HTMLInputElement).value = reason; };
        container.appendChild(chip);
    });
}

function renderSavedReasonsList() {
  const list = document.getElementById('savedReasonsList') as HTMLElement;
  list.innerHTML = '';
...
window.openEditReasonModal = (index: number) => {
  currentReasonIndex = index;
  if (!currentClassId) return;
  (document.getElementById('editReasonText') as HTMLInputElement).value = reasons[currentClassId][index];
  window.openModal('editReasonModal');
}

window.updateReason = () => {
  const newText = (document.getElementById('editReasonText') as HTMLInputElement).value.trim();
...
window.deleteReason = () => {
  if (currentClassId && currentReasonIndex !== '') {
    reasons[currentClassId].splice(currentReasonIndex as number, 1);
    saveData();
    renderSavedReasonsList();
    updateReasonsList();
    window.closeModal('editReasonModal');
...
function renderExtrasView() {
  if (!currentUser) return;
  const gridView = document.getElementById('extrasGridView') as HTMLElement;
  const embedView = document.getElementById('extrasEmbedView') as HTMLElement;
...
window.showExtraTool = (embedId: string) => {
    if (!currentUser) return;
    const gridView = document.getElementById('extrasGridView') as HTMLElement;
    const embedView = document.getElementById('extrasEmbedView') as HTMLElement;
    const schoolEmbeds = embeds[currentUser.school] || [];
...
// Emoji Picker Logic
window.openEmojiPicker = (targetId: string) => {
  emojiPickerTarget = document.getElementById(targetId) as HTMLInputElement;
  (document.getElementById('emojiSearch') as HTMLInputElement).value = '';
  renderEmojiPicker();
  window.openModal('emojiPickerModal');
}

function renderEmojiPicker() {
  const grid = document.getElementById('emojiGrid') as HTMLElement;
  const query = (document.getElementById('emojiSearch') as HTMLInputElement)?.value.toLowerCase().trim() || '';
...
window.selectEmoji = (emoji: string) => {
...
window.resetPassword = () => {
    const username = (document.getElementById('forgotPasswordUsername') as HTMLInputElement).value.trim();
...
window.recoverUsername = () => {
    const email = (document.getElementById('forgotUsernameEmail') as HTMLInputElement).value.trim().toLowerCase();
...
window.onload = () => {
  loadData();
  (document.getElementById('loginBtn') as HTMLElement).onclick = login;
  (document.getElementById('signupBtn') as HTMLElement).onclick = signup;
  (document.getElementById('studentLoginBtn') as HTMLElement).onclick = loginWithPIN;
  (document.getElementById('parentLoginBtn') as HTMLElement).onclick = loginWithParentPIN;
  (document.getElementById('increaseListSizeBtn') as HTMLElement).onclick = increaseListViewSize;
  (document.getElementById('decreaseListSizeBtn') as HTMLElement).onclick = decreaseListViewSize;
...
// Include all admin-specific functions here
// Admin Views
function renderAdminClassesView() {
  if (!currentUser) return;
  const view = document.getElementById('admin-classesView') as HTMLElement;
...
window.openCreateClassModal = () => {
    ['adminClassName', 'teacherUsername', 'teacherPassword', 'teacherEmail', 'presetStudents'].forEach(id => (document.getElementById(id) as HTMLInputElement).value = '');
    window.openModal('createClassModal');
}

window.createClass = () => {
    if (!currentUser) return;
    const className = (document.getElementById('adminClassName') as HTMLInputElement).value.trim();
    const teacherUsername = (document.getElementById('teacherUsername') as HTMLInputElement).value.trim();
    const teacherPassword = (document.getElementById('teacherPassword') as HTMLInputElement).value.trim();
    const teacherEmail = (document.getElementById('teacherEmail') as HTMLInputElement).value.trim().toLowerCase();
    const studentNames = (document.getElementById('presetStudents') as HTMLTextAreaElement).value.trim().split('\n').map(n => n.trim()).filter(n => n);
    ...
    const newClass: Class = { id: generateId(), name: className, teacherUsername: teacherUsername };
    classes.push(newClass);

    if (studentNames.length > 0) {
        students[newClass.id] = {};
        studentNames.forEach(name => {
            const student: Student = { id: generateId(), name, points: 0 };
...
window.openClassDetailModal = (classId: string) => {
    const classInfo = classes.find(c => c.id === classId)!;
    (document.getElementById('classDetailTitle') as HTMLElement).textContent = `Dashboard for ${sanitizeHTML(classInfo.name)}`;
    const content = document.getElementById('classDetailContent') as HTMLElement;
...
window.switchAdminClassDetailTab = (event: Event, classId: string, tab: string) => {
    document.querySelectorAll('#classDetailContent .modal-tab').forEach(t => t.classList.remove('active'));
    (event.currentTarget as HTMLElement).classList.add('active');
    const container = document.getElementById('adminClassDetailTabContent') as HTMLElement;
    container.innerHTML = '';
...
function renderAdminClassRoster(classId: string) {
    const container = document.getElementById('adminClassDetailTabContent') as HTMLElement;
...
function renderAdminUsersView() {
  if (!currentUser) return;
  const view = document.getElementById('admin-usersView') as HTMLElement;
...
window.createTeacher = () => {
    if (!currentUser) return;
    const username = (document.getElementById('newTeacherUsername') as HTMLInputElement).value.trim();
    const password = (document.getElementById('newTeacherPassword') as HTMLInputElement).value.trim();
    const email = (document.getElementById('newTeacherEmail') as HTMLInputElement).value.trim().toLowerCase();
    const allowStudentPins = (document.getElementById('allowStudentPins') as HTMLInputElement).checked;
    const allowParentPins = (document.getElementById('allowParentPins') as HTMLInputElement).checked;
...
window.createAdmin = () => {
    if (!currentUser) return;
    const username = (document.getElementById('newAdminUsername') as HTMLInputElement).value.trim();
    const password = (document.getElementById('newAdminPassword') as HTMLInputElement).value.trim();
    const email = (document.getElementById('newAdminEmail') as HTMLInputElement).value.trim().toLowerCase();
    if (!username || !password || !email) { showNotification('All fields are required.', 'error'); return; }
    if (users[username]) { showNotification('Username already exists.', 'error'); return; }
...
window.openChangeTeacherPasswordModal = (username: string) => {
    currentUsernameToEdit = username;
    (document.getElementById('changeTeacherPasswordTitle') as HTMLElement).textContent = `Change password for ${username}`;
    (document.getElementById('newTeacherPass') as HTMLInputElement).value = '';
    window.openModal('changeTeacherPasswordModal');
}

window.changeTeacherPassword = () => {
    const newPassword = (document.getElementById('newTeacherPass') as HTMLInputElement).value.trim();
...
window.openEditUserModal = (username: string) => {
    currentUsernameToEdit = username;
    const user = users[username];
    (document.getElementById('editUserModalTitle') as HTMLElement).textContent = `Edit ${username}`;
    (document.getElementById('editUserEmail') as HTMLInputElement).value = user.email || '';
    
    const permsDiv = document.getElementById('editUserPermissions') as HTMLElement;
    const photoDiv = document.getElementById('editUserPhotoSection') as HTMLElement;
    if (user.type === 'teacher') {
        permsDiv.style.display = 'block';
        photoDiv.style.display = 'block';
        (document.getElementById('editAllowStudentPins') as HTMLInputElement).checked = user.settings?.allowStudentPins ?? true;
        (document.getElementById('editAllowParentPins') as HTMLInputElement).checked = user.settings?.allowParentPins ?? true;
        const preview = document.getElementById('editUserPhotoPreview') as HTMLElement;
        if (user.photo) {
...
window.updateUser = () => {
    ...
    user.email = (document.getElementById('editUserEmail') as HTMLInputElement).value.trim().toLowerCase();
    if (user.type === 'teacher') {
        user.settings.allowStudentPins = (document.getElementById('editAllowStudentPins') as HTMLInputElement).checked;
        user.settings.allowParentPins = (document.getElementById('editAllowParentPins') as HTMLInputElement).checked;
    }
    saveData();
    renderAdminUsersView();
...
function renderAdminEmbedsView() {
  if (!currentUser) return;
  const view = document.getElementById('admin-embedsView') as HTMLElement;
  const schoolEmbeds = embeds[currentUser.school] || [];
  view.innerHTML = `
...
window.addEmbed = () => {
    if (!currentUser) return;
    const name = (document.getElementById('embedName') as HTMLInputElement).value.trim();
    const url = (document.getElementById('embedUrl') as HTMLInputElement).value.trim();
...
window.removeEmbed = (embedId: string) => {
...
// DEMO MODE
window.startDemoMode = (type: 'admin' | 'teacher') => {
    demoMode = true;
    // Temporarily clear in-memory data for the demo session. Does NOT affect localStorage.
    const tempUsers: { [key: string]: User } = {}; const tempClasses: Class[] = []; const tempStudents: { [key: string]: { [key: string]: Student }} = {}; const tempRewards: { [key: string]: Reward[] } = {}; const tempPowerups: { [key: string]: PowerUp[] } = {}; const tempPointsHistory: { [key: string]: HistoryItem[] } = {}; const tempReasons: { [key: string]: string[] } = {}; const tempEmbeds: { [key: string]: Embed[] } = {};
    ...
    (document.getElementById('demoViewSwitcher') as HTMLElement).style.display = 'flex';
    
    switchDemoView(type);
}

function switchDemoView(type: string) {
    if (!demoMode) return;
    const switcher = document.getElementById('demoViewSwitcher') as HTMLElement;
    switcher.innerHTML = `
    ...
    document.getElementById('studentPortalLayout')?.classList.remove('active');
    document.getElementById('parentPortalLayout')?.classList.remove('active');
    ['loginScreen', 'signupScreen', 'studentLoginScreen', 'parentLoginScreen'].forEach(s => (document.getElementById(s) as HTMLElement).style.display = 'none');
...
window.setupOfflineMode = async (enable: boolean) => {
    if ('serviceWorker' in navigator) {
        ...
        const offlineToggle = document.getElementById('offlineModeToggle') as HTMLInputElement;
...
window.updateProfilePicture = (event: Event) => {
    const file = (event.target as HTMLInputElement).files?.[0];
    if (file && currentUser) {
        const reader = new FileReader();
        reader.onload = (e) => {
            ...
            (document.getElementById('sidebarUserAvatar') as HTMLElement).innerHTML = `<img src="${photoData}" alt="Profile Picture"/>`;
...
window.updateAdminUsername = () => {
    ...
    const newUsername = (document.getElementById('adminUsernameInput') as HTMLInputElement).value.trim();
    if (!newUsername) {
    ...
    if (newUsername !== currentUser.username) {
        ...
        // Update UI
        (document.getElementById('sidebarUsername') as HTMLElement).textContent = newUsername;
        renderSettingsView();
...
window.manageStudentPins = () => {
    if (!currentClassId) return;
    const content = document.getElementById('managePinsContent') as HTMLElement;
    const teacherSettings = users[currentUser!.username]?.settings;
...
window.printPins = () => {
    const content = (document.getElementById('managePinsContent') as HTMLElement).cloneNode(true) as HTMLElement;
